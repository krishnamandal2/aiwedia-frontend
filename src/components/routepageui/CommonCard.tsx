...
const CommonCard = ({
  ...
  // Memoize displayed benefits (max 4)
  const displayedBenefits = useMemo(() => benefits.slice(0, 4), [benefits]);

  // Check overflow with ResizeObserver (better than window resize)
  useEffect(() => {
    const checkOverflow = () => {
  ...
  const imageAlt = `Screenshot of ${title} tool`;
  ...
  return (
    <div className="group relative h-full flex flex-col">
      ...
      <div className="relative bg-white rounded-[1.5rem] border border-slate-200/80 hover:border-indigo-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col h-full overflow-hidden focus-within:ring-2 focus-within:ring-indigo-300">
          ...
          {rank > 0 && (
            <div className="absolute top-2.5 left-2.5 bg-white/95 backdrop-blur-md px-2 py-0.5 rounded-full shadow-sm">
              <span className="text-[10px] font-black text-slate-800">
                <span className="text-[#4F46E5]">#</span>{rank}
              </span>
            </div>
          )}
        </div>
        ...
        <div className="px-5 py-4 flex flex-col flex-grow">
          <div className="mb-3">
            <h3 className="text-base font-bold text-slate-800 group-hover:text-[#4F46E5] transition-colors line-clamp-1 mb-1.5">
              {title}
            </h3>
            ...
            <div className="relative">
              ...
              {isOverflowing && (
                <button
                  ...
                  className="text-[11px] font-bold text-[#4F46E5] mt-1.5 flex items-center gap-1 hover:underline underline-offset-2 transition-all focus:outline-none focus:ring-1 focus:ring-[#4F46E5] rounded-sm"
                >
                  {isExpanded ? (
                    <>Show Less <ChevronUp size={12} aria-hidden="true" /></>
                  ) : (
                    <>Read More <ChevronDown size={12} aria-hidden="true" /></>
                  )}
                </button>
              )}
            </div>
              ...
              {displayedBenefits.map((benefit, i) => (
                <div key={i} className="flex items-center gap-2.5">
                  <div className="w-4 h-4 rounded-md bg-orange-50 flex items-center justify-center shrink-0">
                    <CheckCircle2 size={11} className="text-[#4F46E5]" aria-hidden="true" />
                  </div>
          ...
          <div className="mt-auto pt-2">
            <a
              ...
              className="flex items-center justify-between w-full p-2.5 rounded-xl bg-slate-50 group-hover:bg-[#4F46E5] transition-all duration-300 group/btn focus:outline-none focus:ring-2 focus:ring-[#eb442c] focus:ring-offset-2"
            >
